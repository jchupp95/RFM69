...
void RFM69_readAllRegs() {
  ...
  for (regAddr = 1; regAddr <= 0x4F; regAddr++) {
    ...
    switch (regAddr) {
    case 0x1: {
      Log("Controls the automatic Sequencer ( see section 4.2 "
                  ")\r\nSequencerOff : ");
      if (0x80 & regVal) {
        Log("1 -> Mode is forced by the user\r\n");
      } else {
        Log(
      ...
      capVal = (regVal >> 2) & 0x7;
      if (capVal == 0x0) {
        Log("000 -> Sleep mode (SLEEP)\r\n");
      } else if (capVal == 0b001) {
        Log("001 -> Standby mode (STDBY)\r\n");
      } else if (capVal == 0b010) {
        Log("010 -> Frequency Synthesizer mode (FS)\r\n");
      } else if (capVal == 0b011) {
        Log("011 -> Transmitter mode (TX)\r\n");
      } else if (capVal == 0b100) {
        Log("100 -> Receiver Mode (RX)\r\n");
      } else {
        // Log( capVal, BIN );
    ...
    case 0x2: {
      ...
      if (capVal == 0b00) {
        ...
        Log("10 -> Continuous mode with bit synchronizer\r\n");
      } else if (capVal == 0b11) {
        Log("11 -> Continuous mode without bit synchronizer\r\n");
      }
      ...
      Log("ModulationShaping : ");
      capVal = regVal & 0x3;
      if (modeFSK) {
        if (capVal == 0b00) {
          Log("00 -> no shaping\r\n");
        } else if (capVal == 0b01) {
          Log("01 -> Gaussian filter, BT = 1.0\r\n");
        } else if (capVal == 0b10) {
          Log("10 -> Gaussian filter, BT = 0.5\r\n");
        } else if (capVal == 0b11) {
          Log("11 -> Gaussian filter, BT = 0.3\r\n");
        }
      } else {
        if (capVal == 0b00) {
          Log("00 -> no shaping\r\n");
        } else if (capVal == 0b01) {
          Log("01 -> filtering with f(cutoff) = BR\r\n");
        } else if (capVal == 0b10) {
          Log("10 -> filtering with f(cutoff) = 2*BR\r\n");
        } else if (capVal == 0b11) {
          Log("ERROR - 11 is reserved\r\n");
        }
      }
    ...
    case 0x4: {
      bitRate |= regVal;
      Log("Bit Rate (Chip Rate when Manchester encoding is enabled)\r\nBitRate : ");
    ...
    case 0xa: {
      Log("RC calibration control & status\r\nRcCalDone : ");
      if (0x40 & regVal) {
        Log("1 -> RC calibration is over\r\n");
      } else {
        Log("0 -> RC calibration is in progress\r\n");
      }
    ...
    case 0xb: {
      ...
      if (0x20 & regVal) {
        Log("1 -> Improved AFC routine\r\n");
      } else {
        Log("0 -> Standard AFC routine\r\n");
      }
      Log("\r\n");
      break;
    }
    ...
    case 0xd: {
      uint8_t val;
      Log("Resolution of Listen mode Idle time (calibrated RC osc):\r\nListenResolIdle : ");
      val = regVal >> 6;
      if (val == 0b00) {
        Log("00 -> reserved\r\n");
      } else if (val == 0b01) {
      ...
      if (0x8 & regVal) {
        Log("1 -> signal strength is above RssiThreshold and "
                    "SyncAddress matched\r\n");
      } else {
        Log("0 -> signal strength is above RssiThreshold\r\n");
      }

      Log("\r\nAction taken after acceptance of a packet in Listen mode:\r\nListenEnd : ");
      val = (regVal >> 1) & 0x3;
      if (val == 0b00) {
        Log("00 -> chip stays in Rx mode. Listen mode stops and must be disabled (see section 4.3)\r\n");
      } else if (val == 0b01) {
        Log(
        ...
        Log("10 -> chip stays in Rx mode until PayloadReady or Timeout "
...
void PrintStruct(void) {
  if (datalen != sizeof(dataSTR)) {
    Log("Invalid payload received, not matching data struct!\r\n");
    }
  else {
